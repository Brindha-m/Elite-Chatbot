...
try:
	...
	import timer
	from photoclick import clickPhoto, viewPhoto
	import dictionary
	import ToDo
	import fileHandler
...
try:
	...
	print("You're not Registered Yet !\nRun main.py file to register your account.")
	raise SystemExit
...
def ChangeSettings(write=False):
	...
	if write:
		with open('settings.pck', 'wb') as file:
			pickle.dump(setting, file)
		return
	try:
		with open('settings.pck', 'rb') as file:
...
def changeTheme():
	...
	if themeValue.get()==1:
		...
		background, textColor, AITaskStatusLblBG, KCS_IMG = "#F6FAFB", "#303E54", "#040614", 0
	...
	settingsLbl['fg'], userPhoto['fg'], userName['fg'], assLbl['fg'], volumeLbl['fg'], themeLbl['fg'], chooseChatLbl['fg'] = textColor, textColor, textColor, textColor, textColor, textColor, textColor
	settingsLbl['bg'], userPhoto['bg'], userName['bg'], assLbl['bg'], volumeLbl['bg'], themeLbl['bg'], chooseChatLbl['bg'] = background, background, background, background, background, background, background
...
##################################### DELETE USER ACCOUNT #########################################
def deleteUserData():
	result = messagebox.askquestion('Alert', 'Are you sure you want to delete your Account ?')
	if result=='no': return
	messagebox.showinfo('Clear Account Data', 'Your Account has been cleared\nRegister your account again to use.')
	import shutil
...
def attachTOframe(text,bot=False):
	if bot:
		botchat = Label(chat_frame,text=text, bg=botChatTextBg, fg=botChatText, justify=LEFT, wraplength=250, font=('Aharoni',12, 'bold'))
		botchat.pack(anchor='w',ipadx=5,ipady=5,pady=5)
	else:
		userchat = Label(chat_frame, text=text, bg=userChatTextBg, fg='white', justify=RIGHT, wraplength=250, font=('Aharoni',12, 'bold'))
...
def showSingleImage(type, data=None):
	...
	if type=="weather":
		...
		Label(weather, text=data[0], font=('Aharoni', 45), fg='white', bg='#3F48CC').place(x=65,y=45)
		Label(weather, text=data[1], font=('Aharoni', 15), fg='white', bg='#3F48CC').place(x=78,y=110)
		Label(weather, text=data[2], font=('Aharoni', 10), fg='white', bg='#3F48CC').place(x=78,y=140)
		Label(weather, text=data[3], font=('Aharoni', 12), fg='white', bg='#3F48CC').place(x=60,y=160)
...
def WAEMPOPUP(Service='None', rec='Reciever'):
	...
	Label(PopUProot, text=f'Reciever {rec}', font=('Aharoni', 16), bg='white').pack(pady=(20, 10))
	WAEMEntry = Entry(PopUProot, bd=10, relief=FLAT, font=('Aharoni', 12), justify='center', bg='#DCDCDC', width=30)
	WAEMEntry.pack()
	WAEMEntry.focus()

	SendBtn = Button(PopUProot, text='Send', font=('Aharoni', 12), relief=FLAT, bg='#14A769', fg='white', command=sendWAEM)
...
if __name__ == '__main__':
	...
	splash_label = Label(splash_root, text="Processing...", font=('Aharoni',15),bg='#3895d3',fg='white')
	splash_label.pack(pady=40)
	# splash_percentage_label = Label(splash_root, text="0 %", font=('Aharoni',15),bg='#3895d3',fg='white')
	...
	cbl.pack(pady=17)
	AITaskStatusLbl = Label(VoiceModeFrame, text='    Offline', fg='white', bg=AITaskStatusLblBG, font=('Aharoni', 13))
	AITaskStatusLbl.place(x=140,y=36)
	...
	settingBtn = Button(VoiceModeFrame,image=sphimage,height=40,width=40, bg='#dfdfdf',borderwidth=0,activebackground="#dfdfdf",command=lambda: raise_frame(root2))
	...
	#Mic
	micImg = PhotoImage(file = "extrafiles/images/mic1.png")
	micImg = micImg.subsample(2,2)
	micBtn = Button(TextModeFrame,image=micImg,height=42,width=42, bg='#dfdfdf',borderwidth=0,activebackground="#dfdfdf", command=changeChatMode)
	micBtn.place(relx=1.0, y=25,x=-20, anchor="ne")	
	...
	UserFieldLBL.pack(pady=10, side=LEFT, padx=19)
	UserField = Entry(TextModeFrame, fg='white',bg='#050a26', font=('Aharoni', 13), bd=10, width=33, relief=FLAT)
	UserField.place(x=49, y=29)
	...
	settingsLbl = Label(root2, text='Settings', font=('Aharoni', 15), bg=background, fg=textColor)
	...
	#Username
	userName = Label(root2, text=ownerName, font=('Aharoni', 15), fg=textColor, bg=background)
	userName.pack()
	...
	assLbl = Label(settingsFrame, text='Assistant Voice', font=('Aharoni', 13), fg=textColor, bg=background)
	assLbl.place(x=0, y=20)
	n = StringVar()
	assVoiceOption = ttk.Combobox(settingsFrame, values=('Female', 'Male'), font=('Aharoni', 13), width=13, textvariable=n)
	assVoiceOption.current(voice_id)
	assVoiceOption.place(x=150, y=20)
	assVoiceOption.bind('<<ComboboxSelected>>', changeVoice)

	
	volumeLbl = Label(settingsFrame, text='Volume', font=('Aharoni', 13), fg=textColor, bg=background)
	...
	themeLbl = Label(settingsFrame, text='Theme', font=('Aharoni', 13), fg=textColor, bg=background)
	themeLbl.place(x=0,y=143)
	themeValue = IntVar()
	s = ttk.Style()
	s.configure('Wild.TRadiobutton', font=('Aharoni', 10), background=background, foreground=textColor, focuscolor=s.configure(".")["background"])
	...
	chooseChatLbl = Label(settingsFrame, text='Chat Background', font=('Aharoni', 13), fg=textColor, bg=background)
	...
	backBtn = Button(settingsFrame, text='   Back   ', bd=0, font=('Aharoni 12'), fg='white', bg='#14A769', relief=FLAT, command=lambda:raise_frame(root1))
	clearaccBtn = Button(settingsFrame, text='   Delete Account   ', bd=0, font=('Aharoni 12'), fg='white', bg='#14A769', relief=FLAT, command=deleteUserData)
	backBtn.place(x=5, y=250)
	clearaccBtn.place(x=120, y=250)
